{
    "description": "add images tab",
    "active": true,
    "record": true,
    "code": function(s_record) {
                ...
                s_record.tabs.addTab('Images', {
                    ...
                    click: function(event, tab, record, options) {
                        if (tab.isOpen()) {
                            ...
                            var tab_content = '';
                            var image_records = jQuery.PRIMO.search.byQuery('lsr05,exact,MPLUS_' + record.id, {
                                bulkSize: 500
                            });
                            var image_data = [];
                            ...
                            if (image_records) {
                                ...
                                jQuery.each(image_records, function(index, data) {
                                    ...
                                    if (jQuery.isArray(data.LINKS.linktorsrc)) {
                                        linktorsrc = jQuery.grep(data.LINKS.linktorsrc, function(a) {
                                            return (a.search('carmentis') != -1)
                                        })[0];
                                    } else {
                                    ...
                                    if (jQuery.isArray(data.LINKS.backlink)) {
                                        backlink = jQuery.grep(data.LINKS.backlink, function(a) {
                                            return ((a.search('carmentis') != -1) && (a.search('lang=' + language) != -1))
                                        })[0];
                                    } else {
                            ...
                            tab.open(tab_content, {
                                reload: true
                            });

                            jQuery('#imageContainer-' + record.index).photobox('a', {
                                time: 0,
                                zoomable: false,
                                thumbs: false
                            });
